---
data:
  - matkul: "Sistem Hidrolik dan Pneumatik, Mekanika Fluida, Fisika dan Kimia Dasar "
    matkulDiPsLain: Fisika dan Kimia Dasar, Jaringan Komputer II (PS T. Elektronika)
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "1103027702"
    nama: Ali Abrar, S.Si., M.T.
    bidangKeahlian: Teknik Mesin
    jabatan: 1
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: "18100503200407"
    sesuaiKompetensi: true
  - matkul: 1. Mekanika Teknik, 2. Motor Bakar II, 3. Hand and Measurement Tools, 4.
      Ilmu Bahan II
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0028088502"
    nama: Zulkifli, S.T., M.T.
    bidangKeahlian: Teknik Mesin
    jabatan: 2
    namaProdi:
      s2: Teknik Mesin
    noSertifikatPendidik: "17100503202665"
    sesuaiKompetensi: true
  - matkul: >-
      Sistem Penyejuk Udara, Chasis dan Under Carriage, Rangkaian Pemindah
      Tenaga, Dasar-Dasar

      Hidrolik dan Pneumatik
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0001046214"
    nama: Drs. Syaeful Akbar, M.T.
    bidangKeahlian: Manajemen Industri
    jabatan: 2
    namaProdi:
      s2: Manajemen  Industri
    noSertifikatPendidik: "14100503203411"
    sesuaiKompetensi: true
  - matkul: Konversi Energi, Manajemen Bengkel, K3L, Pesawat Angkat, Fisika dan
      Kimia Dasar
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0031127417"
    nama: Ir. Ida Bagus Dharmawan, S.T., M.Si.
    bidangKeahlian: Ilmu Lingkungan
    jabatan: 2
    namaProdi:
      s2: "Ilmu Lingkungan "
    noSertifikatPendidik: "14100503203746"
    sesuaiKompetensi: false
  - matkul: Thermodinamika, Sistem Bahan Bakar, Pesawat Angkat, Motor Bakar I
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0006028206"
    nama: Subur Mulyanto, S.Pd., M.T.
    bidangKeahlian: Teknik Mesin
    jabatan: 2
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: "18100503205980"
    sesuaiKompetensi: true
  - matkul: "Matematika Teknik II, Statistik, Matematika Teknik I, Aplikasi Komputer "
    matkulDiPsLain: "Matematika Teknik (PS Teknologi Listrik dan Teknik Elektronika) "
    sertifikat:
      bidangSertifikasi: ""
      lembagaPenerbit: ""
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "19068304"
    nama: Dr. Hadi Hermansyah, S.Si., M.Si
    bidangKeahlian: Ilmu Kelautan
    jabatan: 2
    namaProdi:
      s3: Ilmu Kelautan
      s2: Ilmu Kelautan
    noSertifikatPendidik: "17100503200723"
    sesuaiKompetensi: false
  - matkul: "Analisa Kerusakan, Ilmu Bahan I, Pengenalan dan Penggunaan Alat Berat,
      Ilmu Bahan II, Manejemen Perawatan Alat Berat "
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0017058103"
    nama: Ir. Wahyu Anhar, S.T., M.Eng.
    bidangKeahlian: Teknik Mesin
    jabatan: 2
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: "17100503205153"
    sesuaiKompetensi: true
  - matkul: Bahasa Inggris I, Bahasa Inggris II, Bahasa Inggris III
    matkulDiPsLain: "Bahasa Indonesia, Bahasa Inggris Perhotelan dan Bahasa Inggris Profesi "
    sertifikat:
      bidangSertifikasi: ""
      lembagaPenerbit: ""
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0010068901"
    nama: Elisabeth Milaningrum, S.Pd., M.Pd.
    bidangKeahlian: Pendidikan Bahasa Inggris
    jabatan: 2
    namaProdi:
      s3: ""
      s2: Pendidikan Bahasa Inggris
    noSertifikatPendidik: "180010330057"
    sesuaiKompetensi: false
  - matkul: Bahasa Indonesia, Pancasila, Etika Profesi, Kewarganegaraan, Kewirausahaan
    matkulDiPsLain: "Pancasila, Kewarganegaraan (PS Keuangan Perbankan),
      Kewarganegaraan (PS Elektronika) "
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0031017601"
    nama: Patria Rahmawaty, S.Pd., M.M.Pd.,Psikolog.
    bidangKeahlian: Manajemen Sekolah
    jabatan: 2
    namaProdi:
      s3: ""
      s2: Manajemen Sekolah
    noSertifikatPendidik: "18100503204779"
    sesuaiKompetensi: true
  - matkul: Agama
    matkulDiPsLain: "Agama (PS Tata Boga, Divisi Kamar, Keuangan Perbankan, Teknik
      Listrik, Teknik Elektronika, Teknik Sipil, Teknik Konstru ksi Jalan dan
      Jembatan) "
    sertifikat:
      bidangSertifikasi: ""
      lembagaPenerbit: ""
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "1171087101"
    nama: Basri Dahlan, S.Ag., M.Pd.I
    bidangKeahlian: Pendidikan Islam
    jabatan: 2
    namaProdi:
      s2: Pendidikan Islam
    noSertifikatPendidik: "15100503200709"
    sesuaiKompetensi: false
  - matkul: Mata Kuliah
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: ""
      lembagaPenerbit: ""
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0013058906"
    nama: Yudi Kurniawan, S.T., M.T.
    bidangKeahlian: Teknik Mesin
    jabatan: 1
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: ""
    sesuaiKompetensi: true
  - matkul: Mata Kuliah
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: ""
      lembagaPenerbit: ""
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "-"
    nama: Arwin, S.Pd., M.T,
    bidangKeahlian: Teknik Mesin
    jabatan: 1
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: ""
    sesuaiKompetensi: true
  - matkul: Mata Kuliah
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: ""
      lembagaPenerbit: ""
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "-"
    nama: Aryati Muhaymin Marali, S.Pd., M.T.
    bidangKeahlian: Teknik Mesin
    jabatan: 1
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: ""
    sesuaiKompetensi: true
  - matkul: Mata Kuliah
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: ""
      lembagaPenerbit: ""
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "1105099501"
    nama: Herdian Dwimas, S.T., M.T.
    bidangKeahlian: Teknik Mesin
    jabatan: 1
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: ""
    sesuaiKompetensi: true
  - matkul: Mata Kuliah
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: ""
      lembagaPenerbit: ""
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "0019099502"
    nama: Yasmin Zulfati Yusrina, S.T., M.T.
    bidangKeahlian: Teknik Mesin
    jabatan: 1
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: ""
    sesuaiKompetensi: true
  - matkul: Sistem Hidrolik dan Pneumatik, Manajemen Mutu, Gambar Teknik Mesin,
      Dasar-Dasar Hidrolik dan Pneumatik
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "1174117401"
    nama: Dr. Syahruddin, S.Pd., M.T.
    bidangKeahlian: Manajemen Industri
    jabatan: 2
    namaProdi:
      s2: Manajemen  Industri
      s3: Teknik Mesin
    noSertifikatPendidik: "15100503209430"
    sesuaiKompetensi: true
  - matkul: Praktik Bengkel I, Praktik Bengkel II, Dasar-dasar kelistrikan dan
      Elektronika, Sistem Kelistrikan Alat Berat
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "9900979713"
    nama: Mohamad Amin, S.Pd.T., M.Pfis.
    bidangKeahlian: Pengajaran Fisika
    jabatan: 1
    namaProdi:
      s3: ""
      s2: Pengajaran Fisika
    noSertifikatPendidik: ""
    sesuaiKompetensi: false
  - matkul: "Kinematik dan Dinamik, Electric Drive and Electronica Control System,
      Mekatronika, Elemen Mesin "
    matkulDiPsLain: ""
    sertifikat:
      bidangSertifikasi: Bidang Sertifikasi
      lembagaPenerbit: Lembaga Penerbit
    bidangKeahlianSesuaiMatkul: true
    nomorInduk: "002410860"
    nama: Randis, S.T., M.T.
    bidangKeahlian: Teknik Mesin
    jabatan: 1
    namaProdi:
      s3: ""
      s2: Teknik Mesin
    noSertifikatPendidik: "0024108601"
    sesuaiKompetensi: true
---

<script setup>
import { useData } from "vitepress"
import Tabel from '../components/tabel-3a1.vue'

const { frontmatter } = useData()
</script>

# Dosen Tetap Perguruan Tinggi yang Ditugaskan sebagai Pengampu Mata Kuliah di Program Studi yang Diakreditasi

Tuliskan data Dosen Tetap Perguruan Tinggi yang ditugaskan sebagai pengampu mata kuliah di Program Studi yang diakreditasi (DTPS) pada saat TS.

## Tabel 3.a.1

<Tabel :data="frontmatter.data" />

::: info KETERANGAN

NDT = Jumlah Dosen Tetap Perguruan Tinggi yang ditugaskan sebagai pengampu mata kuliah di Program Studi yang diakreditasi.

NDTPS = Jumlah Dosen Tetap Perguruan Tinggi yang ditugaskan sebagai pengampu mata kuliah dengan bidang keahlian yang sesuai dengan kompetensi inti program studi yang diakreditasi.

1. Diisi dengan nama Program Studi pada pendidikan pasca sarjana (Magister/Magister Terapan dan/atau Doktor/Doktor Terapan) yang pernah diikuti.
1. Diisi dengan bidang keahlian sesuai pendidikan pasca sarjana yang relevan dengan mata kuliah yang diampu.
1. Diisi dengan tanda centang V jika bidang keahlian sesuai dengan kompetensi inti Program Studi yang diakreditasi.
1. Diisi dengan nomor Sertifikat Pendidik Profesional.
1. Diisi dengan bidang sertifikasi dan lembaga penerbit sertifikat.
1. Diisi dengan nama mata kuliah yang diampu pada Program Studi yang diakreditasi pada saat TS-2 s.d. TS.
1. Diisi dengan tanda centang V jika bidang keahlian sesuai dengan mata kuliah yang diampu.
1. Diisi dengan nama mata kuliah yang diampu pada Program Studi lain pada saat TS-2 s.d. TS.

:::
